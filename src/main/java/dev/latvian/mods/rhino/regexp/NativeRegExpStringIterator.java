...
public final class NativeRegExpStringIterator extends ES6Iterator {
	...
	public static void init(ScriptableObject scope, boolean sealed, Context cx) {
		ES6Iterator.init(scope, sealed, new NativeRegExpStringIterator(), ITERATOR_TAG, cx);
	}
	...
	protected boolean isDone(Context cx, Scriptable scope) {
		...
		// value here and return it from "nextValue".
		...
		String matchStr = ScriptRuntime.toString(cx, ScriptRuntime.getObjectIndex(cx, scope, next, 0));
		if (matchStr.isEmpty()) {
			long thisIndex = ScriptRuntime.toLength(cx, ScriptRuntime.getObjectProp(cx, scope, regexp, "lastIndex"));
			long nextIndex = ScriptRuntime.advanceStringIndex(string, thisIndex, fullUnicode);
			ScriptRuntime.setObjectProp(cx, scope, regexp, "lastIndex", nextIndex);
		}
	...
	private Object regExpExec(Context cx, Scriptable scope) {
		...
		Object execMethod = ScriptRuntime.getObjectProp(cx, scope, regexp, "exec");
